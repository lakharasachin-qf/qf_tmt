...
//        holder.binding?.obj = list[position]
        holder.binding!!.tvGuest.text = (position + 1).toString()
        if (isAdult && !firstTime) {
            flags[0] = true // 0 = true
            firstTime = true
        }
        if (flags[holder.layoutPosition]!!) { //0 == true
            if (isAdult) {
                previousPosition = holder.layoutPosition
            }
        ...
        holder.binding!!.tvGuest.setOnClickListener {
            ...
            if (isAdult) {
                flags[previousPosition] = false
                flags[holder.layoutPosition] = true
            } else {
                flags[holder.layoutPosition] = !flags[holder.layoutPosition]!!
            }
